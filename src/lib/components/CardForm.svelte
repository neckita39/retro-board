	...
	<form onsubmit={(e) => { e.preventDefault(); submit(); }} class="space-y-2">
		...
		{#if imagePreview}
			<div class="relative inline-block">
				<img src={imagePreview} alt="" class="max-h-24 rounded-lg border border-border object-cover {uploading ? 'opacity-50' : ''}" />
				{#if uploading}
					<span class="absolute inset-0 flex items-center justify-center text-xs font-medium text-text-secondary">{t('image.uploading')}</span>
				{/if}
				<button
					type="button"
					onclick={removeImage}
