...
- **Blind writing**: `boards.blind` (per board, toggled by the creator via socket `board:blind`, guarded by `isRoomCreator` like the timer). While on, a card is visible only to its author — everyone else gets a face-down placeholder that keeps its place in the column. Authorship is `cards.author_session`, the same browser id as votes (`retro-session-id`), sent on `board:join` and kept in `socket.data.session`; **it never leaves the server**, not even to its owner, or cards could be grouped by author. Masking is `blind.js` **in the repo root** (Dockerfile COPY line, typed re-export `src/lib/blind.ts`): `viewCard` also strips `author_session` when the mode is off. `card:created` / `card:updated` are emitted per socket via `fetchSockets()`, and toggling re-sends `cards:state` to each viewer. Comments of hidden cards are withheld too, and mutations on someone else's face-down card are refused. The SvelteKit load hides *everything* while blind (it cannot know the viewer's session — that lives in localStorage); the socket then reveals the viewer's own. The export is masked the same way, otherwise the mode is bypassed with one link
- **Board search**: the magnifier in the board header (or the `/` hotkey — ignored while typing in an input) opens `BoardSearch.svelte`, a bar under the header. Matching logic is pure and unit-tested in `src/lib/search.ts` (`cardMatches` — card text, author and comments; every word must occur, order does not matter; case- and ё/е-insensitive). State is `searchStore` (`src/lib/stores/search.svelte.ts`), memory only — never the server or localStorage, and reset when the board changes. Cards are dimmed, never hidden: on a retro it matters which column the hit is in. A hit carries `data-search-hit`. `/api` has its own list filter over titles, descriptions and paths
