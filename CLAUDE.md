...
- Notifications: one system — `toastStore.push({ kind, text, action?, timeoutMs? })` (`action` = `{ label, href }` or `{ label, onClick }`), rendered by `Toasts.svelte` top-right. The «board/space created — copy link» notice is such a toast (copies the public URL, `?admin` stripped); there is no bottom banner component
