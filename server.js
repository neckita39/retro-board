...
const httpServer = createServer(async (req, res) => {
	...
	// --- Image upload (bypass SvelteKit body limit) ---
			...
			try {
				...
				if (!detectedMime) {
					for (const { bytes, mime } of MAGIC) {
						if (fileBuffer.length >= bytes.length && bytes.every((b, i) => fileBuffer[i] === b)) {
							detectedMime = mime;
							break;
						}
					}
				...
				if (detectedMime === 'image/gif') {
					result = await sharp(fileBuffer, { animated: true })
						.resize(1200, 1200, { fit: 'inside', withoutEnlargement: true })
						.gif()
						.toBuffer({ resolveWithObject: true });
					result.mimeType = 'image/gif';
				} else {
					result = await sharp(fileBuffer)
						.resize(1200, 1200, { fit: 'inside', withoutEnlargement: true })
						.webp({ quality: 80 })
						.toBuffer({ resolveWithObject: true });
				...
				res.writeHead(200, { 'Content-Type': 'application/json' });
				res.end(JSON.stringify({ imageId: image.id, width: result.info.width, height: result.info.height }));
			} catch (err) {
